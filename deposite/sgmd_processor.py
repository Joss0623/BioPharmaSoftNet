...
References:
[1] Pan Haiyang. Mechanical Fault Diagnosis Method Based on Symplectic Geometry Mode Decomposition and Support Matrix Machine [D]. Hunan University, 2019.
...
class SGMD:
    def __init__(self, sig, fs, mode='eig', delay_time=1, nfft=256, is_plot=False, threshold_corr=0.95,
                 threshold_nmse=0.25, end_indicator='nmse') -> None:
        """
        Initialization of SGMD parameters.

        Parameters:
            sig (ndarray): Target signal
            fs (int): Sampling frequency of the signal
            mode (str): 'eig', 'schur' or 'qr' method for matrix decomposition (default 'eig')
            delay_time (int): Delay time parameter for trajectory matrix (default 1)
            nfft (int): Window length for PSD calculation (default 256)
            is_plot (bool): Whether to plot the PSD figure (default False)
            threshold_corr (float): Correlation threshold for mode grouping (default 0.95)
            threshold_nmse (float): Normalized mean square error threshold to stop iteration (default 0.25)
            end_indicator (str): Metric to decide stopping criterion, one of 'nmse', 'std', or 'var' (default 'nmse')
        """
        self.sig = sig
        self.n = len(sig)
        self.fs = fs
    ...
    def trajectory_matrix(self, is_plot=False, delay_time=1):
        """
        Construct the trajectory matrix of the signal based on delay embedding.

        Args:
            is_plot (bool): Whether to plot the power spectral density (PSD)
            delay_time (int): Delay time for embedding

        Returns:
            self.X (ndarray): Trajectory matrix
        """
        if 'X' in dir(self):
            if is_plot:
    ...
    def sgmd(self):
        """
        Perform Symplectic Geometry Mode Decomposition (SGMD) on the input signal.

        Returns:
            SGCs (ndarray): Extracted symplectic geometry components (modes)
        """
        ...
        for i in range(self.d):
            matrix_Z = np.dot(np.dot(np.expand_dims(matrix_Q[:, i], -1), np.expand_dims(matrix_Q[:, i], -1).T),
                              self.X.T)
            ...
            matrix_Y[i, self.d:self.m] = [np.mean(np.diag(np.flip(matrix_Z[:, j + 1 - self.d:j + 1], axis=1))) for j in
                                          range(self.d, self.m)]
            matrix_Y[i, self.m:] = [np.mean(np.diag(np.flip(matrix_Z[j + 1 - self.m:, j + 1 - self.d:], axis=1))) for j
                                    in range(self.m, self.n)]
        ...
        while len(index[flags]):
            ...
            SGCs = np.vstack(
                (SGCs, source + np.sum(matrix_Y[tmp_flag], axis=0))) if 'SGCs' in dir() else source + np.sum(
                matrix_Y[tmp_flag], axis=0)
            if self.end_indicator == 'nmse':
...
if __name__ == '__main__':
    fs = 5120  # Sampling frequency (Hz)
    time = 1  # Duration (seconds)
    t = np.arange(0, time, 1 / fs)

    # Construct composite signal with multiple frequency components
