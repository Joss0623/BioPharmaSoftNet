...
lambda_value = 1e-2
exp_itme = "nd_4"  # Modify this to project_IMFcount only, no other changes required
type_names_list = ["jn", "nd", "ht"]

# Dataset key dictionary (desensitized)
tensorset_dic = {
    "jn_3": 0,
    "jn_4": 1,
    "jn_5": 2,
...
def load_data(type_name: str, num_imf: int, file_path="./Data/DYG_sgc_pred.xlsx") -> TensorDataset:
    try:
        ...
        data_dic = {}
        # Desensitized: columns must be pre-processed to remove sensitive info if any
        ...
        print(f"Error loading data for {type_name}: {e}")
...
# Load all data at once for batch processing

dataset = data
train_size = int(0.8 * len(dataset))  # 80% training split
...
for epoch in range(200):  # Train for 200 epochs
    ...
    for batch in train_loader:
        ...
        # loss += lambda_value * torch.sum(weights ** 2)
        loss.backward()
        optimizer.step()
...
# Iterate over test dataset and concatenate tensors per sample
for i in range(len(test_dataset)):
    single_sample = test_dataset[i]
    single_sample_concatenated = torch.cat(single_sample, dim=0)
    test_data_list.append(single_sample_concatenated)

# Stack all test tensors into one big tensor
...
# Manual weight adjustment example (optional)
